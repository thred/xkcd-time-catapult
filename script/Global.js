	...
	return {
		...
		activateUnstableParticles: function() {
			...
			do {
				...
				for (var idx in this.UNSTABLE_PARTICLES) {
					...
					var rnd = Math.random();
					var offset = (Math.random() - 0.5) * 2 * 10;

					sand = sand * this.PARTICLE_MASS_PER_PIXEL;

					if (this.SAND.get(x, y + 1) <= this.PARTICLE_THRESHOLD) {
						// there's no sand below
						this.addParticle(new Particle(new Vector(x, y), new Vector(offset, 0), sand));
						this.SAND.remove(x, y, sand * this.PARTICLE_MASS_PER_PIXEL);
						this.unstableParticle(x, y - 1);
					} else if ((rnd < 0.33) && (this.SAND.get(x - 1, y + 1) <= this.PARTICLE_THRESHOLD)) {
						// there's no sand below left
						this.addParticle(new Particle(new Vector(x, y), new Vector(-15 + offset, 0), sand));
						this.SAND.remove(x, y, sand * this.PARTICLE_MASS_PER_PIXEL);
						this.unstableParticle(x, y - 1);
					} else if ((rnd >= 0.66) && (this.SAND.get(x + 1, y + 1) <= this.PARTICLE_THRESHOLD)) {
		...
		activatePendingParticles: function() {
			...
			for (var i = 0; i < this.PENDING_PARTICLES.length; i += 1) {
				...
				this.SAND.remove(pendingParticle.position.x, pendingParticle.position.y, pendingParticle.mass);
